...
with open('./mcnn_pred/mcnn_predictions.csv', 'r') as f:
    ...
    #     normal:   positive
    #     abnormal: negative
    true_pos = np.sum(np.logical_and(pred == 'normal', true == 'normal'))
    false_pos = np.sum(np.logical_and(pred == 'normal', true == 'anomaly'))
    true_neg = np.sum(np.logical_and(pred == 'anomaly', true == 'anomaly'))
    false_neg = np.sum(np.logical_and(pred == 'anomaly', true == 'normal'))
