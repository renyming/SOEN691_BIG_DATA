...
class MC_NN:
    ...
    def __init__(self, theta):
        ...
        self.mx = 10    # maximum number of mcs
    ...
    def read_mcnn_pool(self):
        ...
        for file in mc_files:
            ...
            with open(join(mc_folder, file), 'r') as f:
                ...
                mc.centroid = [float(x) for x in lines[2].split(',') if
                               is_number(x)]
                mc.cf2_x = [float(x) for x in lines[3].split(',')]
                mc.status = lines[4]  # status
                mc.filename = lines[5]

            mc.cf1_x = [x * mc.n for x in
                        mc.centroid]  # The sum of the feature is the average centroid * n
    ...
    def split_mc(self):
        ...
        for mc in self.pool:
            ...
            if (mc.epsilon > self.theta):
                ...
                child_mc_1.cf2_x = [float(x) * float(y) for x, y in
                                    zip(child_mc_1.cf1_x, child_mc_1.cf1_x)]
                child_mc_2.cf2_x = [float(x) * float(y) for x, y in
                                    zip(child_mc_2.cf2_x, child_mc_2.cf2_x)]
...
def init_mcnn_pool(data_file, sc):
    ...
    while normal is None or anomaly is None:
        rand = data.takeSample(withReplacement=False, num=1,
            seed=random.randint(0, 100))[0]

        if rand[-1] == 'normal' and normal is None:
            normal = rand

            cf1_x_normal = [x for x in normal if is_number(x)]
            cf2_x_normal = [float(x) * float(y) for x, y in
                            zip(cf1_x_normal, cf1_x_normal)]

        elif rand[-1] == 'anomaly' and anomaly is None:
            anomaly = rand
            cf1_x_anomaly = [x for x in anomaly if is_number(x)]
            cf2_x_anomaly = [float(x) * float(y) for x, y in
                             zip(cf1_x_anomaly, cf1_x_anomaly)]

    with open(join(mc_folder, 'normal_mc_1.csv'), 'w', newline='') as f:
        csv_writer = csv.writer(f)
        csv_writer.writerow("0")  # initial epsilon #first row
        csv_writer.writerow(
            "1")  # initial count of instances in the cluster #second row
        csv_writer.writerow(
            normal)  # initial centroid in the cluster #third row
    ...
    with open(join(mc_folder, 'anomaly_mc_1.csv'), 'w', newline='') as f:
        ...
        csv_writer.writerow(
            "1")  # initial count of instances in the cluster #second row
        csv_writer.writerow(
            anomaly)  # initial centroid in the cluster #third row
...
def predict(instance):
    ...
    (prediction, true_lable, time_stamp) = mcnn.predict_and_update_mcs(
        instance, instance[-1])
